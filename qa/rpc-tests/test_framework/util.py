...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "title.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def wait_for_titled_start(process, url, i):
    '''
    Wait for titled to start. This means that RPC is accessible and fully initialized.
    Raise an exception if titled exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception(
                'titled exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir, num_nodes, cachedir):
    ...
    if create_cache:
        ...
        # Create cache directories, run titleds:
        for i in range(MAX_NODES):
            datadir = initialize_datadir(cachedir, i)
            args = [os.getenv("TITLED", "titled"), "-server",
                    "-keypool=1", "-datadir=" + datadir, "-discover=0"]
            if i > 0:
                args.append("-connect=127.0.0.1:" + str(p2p_port(0)))
            titled_processes[i] = subprocess.Popen(args)
            logger.debug("Creating data directories from cached datadir")
            wait_for_titled_start(titled_processes[i], rpc_url(i), i)
    ...
    for i in range(num_nodes):
        ...
        # Overwrite port/rpcport in title.conf
...
def locate_titled_binary():
    """
    Find titled binary if possible.
    """
    titled_binary = os.getenv("TITLED", DEFAULT_TITLED)
    if os.path.exists(titled_binary):
        return titled_binary

    if os.path.exists(os.path.join('src', DEFAULT_TITLED)):
        titled_binary = os.path.abspath(
            os.path.join('src', DEFAULT_TITLED))
    elif titled_binary == 'titled' or not os.path.exists(titled_binary):
        # If TITLED was specified and exists, use it, otherwise look for source.
        # get_srcdir() already returns an absolute path
        src_dir_cand = get_srcdir(sys.argv[0])
        if src_dir_cand and os.path.exists(
                os.path.join(src_dir_cand, 'src', DEFAULT_TITLED)):
            titled_binary = os.path.join(
                src_dir_cand, 'src', DEFAULT_TITLED)
        else:
            sys.stderr.write("Unable to locate titled for this test.\n")
            sys.exit(1)
    return titled_binary
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, stderr_checker=None):
    """
    Start a titled and return RPC connection to it.
    If stderr_checker is provided, it must be an OutputChecker.
    Its output_file_obj will be connected to the stderr of the titled process.
    """
    datadir = os.path.join(dirname, "node" + str(i))
    if binary is None:
        binary = locate_titled_binary()
    ...
    if stderr_checker:
        ...
        titled_processes[i] = subprocess.Popen(args)
    logger.debug(
        "initialize_chain: titled started, waiting for RPC to come up")
    url = rpc_url(i, rpchost)
    wait_for_titled_start(titled_processes[i], url, i)
    logger.debug("initialize_chain: RPC successfully started")
...
def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, stderr_checkers=None):
    """
    Start multiple titleds, return RPC connections to them
...
def stop_node(node, i):
    ...
    return_code = titled_processes[i].wait(
        timeout=TITLED_PROC_WAIT_TIMEOUT)
    assert_equal(return_code, 0)
    del titled_processes[i]


def stop_nodes(nodes):
    for i, node in enumerate(nodes):
        stop_node(node, i)
    assert not titled_processes.values()  # All connections must be gone now
