...
'''
Test plan:
- Start titled's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on titled side:
...
- Create titleds that connect to them
- Manipulate the titleds using addnode (onetry) an observe effects
...
class ProxyTest(BitcoinTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: titled's SOCKS5 implementation only sends atyp DOMAINNAME,
        ...
        if self.have_ipv6:
            ...
            assert(isinstance(cmd, Socks5Command))
            # Note: titled's SOCKS5 implementation only sends atyp
